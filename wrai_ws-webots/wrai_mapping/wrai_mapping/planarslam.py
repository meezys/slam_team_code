...
class GraphSLAM:
    ...
    def __init__(
        ...
        self.landmark_counter = 0  # Add a counter for unique landmark keys
        #print(self.lm_tree)
    ...
    def optimise(self,estimates) -> np.ndarray:
        ...
        # Check the type of value associated with the key
            if self.isam.valueExists(key):
                #value = estimates.at(key)
                print(f"Key {key} already exists in ISAM2 values.")
                continue
        #print(self.graph)"""
    ...
    def add_landmark_estimate(self,pose, symbol):
        if not self.estimates.exists(symbol):
            self.estimates.insert(symbol, gtsam.Point2(pose[0], pose[1]))
